...
管理员打开 `admin.html` 发布链接；其他人打开 `index.html`，在页面上方填写一次姓名，然后打开全部链接，最后点击页面下方的“提交签到”一次。截图上传为可选项，每个链接不再重复填写姓名。
...
飞书 App Secret 只能配置在 Worker 的服务端环境变量中，不能上传到 GitHub。部署完成后，管理员页面发布的链接和用户签到记录会保存到飞书多维表格；用户选择上传截图时，截图会写入 `checkins` 表的“截图”附件字段。管理员可以在管理端导出签到记录为 Excel 可打开的 CSV 文件；导出成功的记录会在北京时间每天 02:00 自动清理，未导出的记录会保留。
